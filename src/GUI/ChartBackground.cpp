...
void drawChartBackground(sf::RenderWindow& window, ChartBackground* background){
    ...
    if (offsetX >= 0) {
        xLeft = startX + background->rect.width / 2 - std::fmod(offsetX, addX);
        xRight = startX + background->rect.width / 2 + (addX - std::fmod(offsetX, addX));
    }
    else
    {
        xLeft = startX + background->rect.width / 2 - (addX - std::fmod(-offsetX, addX));
        xRight = startX + background->rect.width / 2 + std::fmod(-offsetX, addX);
    }


    if (offsetY <= 0) {
        yTop = startY + background->rect.height / 2.0 + (addY - std::fmod(offsetY, addY));
        yBottom = startY + background->rect.height / 2 - std::fmod(offsetY, addY);
    }
    else {
        yTop = startY + background->rect.height / 2.0 + std::fmod(-offsetY, addY);
        yBottom = startY + background->rect.height / 2.0 - (addY - std::fmod(-offsetY, addY));
    }
    /*setcolor(BLACK);
